...
=======
        ...
        # self.leftLight.node().setShadowCaster(True, 512, 512)
        ...
        # self.rightLight.node().setShadowCaster(True, 512, 512)
...
class CheckpointBox:
    ...
    def __init__(self, world, position=(0, 10, 0), scale=(5, 2, 1), nodeName="Ghost", gateID=-1):
        globals.checkpointObjects.append(self)
...
>>>>>>> Stashed changes
    ...
    def camera_task(self, *args, **kwargs):
        ...
        position = self.test_car.chassisNP.getPos(render)
        rotation = self.test_car.chassisNP.getHpr(render)

        # print(rotation)

        base.cam.setPos(position[0]+(10*math.sin(rotation[0]/57.5)), position[1]-(10*math.cos(rotation[0]/57.5)), position[2]+3)

        base.cam.setHpr(-rotation[0], -rotation[1], -rotation[2])
        base.cam.lookAt(position[0], position[1], position[2]+0.2)
...
>>>>>>> Stashed changes
        ...
        geomNodes = loader.loadModel((globals.rel_path(None, path="/src/models/tracks/Road ML TT1SSNM.bam"))).findAllMatches('**/+GeomNode')
        ...
        self.light = loader.loadModel("src/models/other/Light placeholderb2b.bam")
        self.light.setColor((1, 0, 0, 1))
        self.light.setPos(0, 5, 0)
        # self.light.setScale(.25)
        # self.light.reparentTo(self.chassisNP)
        self.Light = self.light.attachNewNode(
            PointLight("light"))
        self.Light.node().setColor((.35, 0.8, 0.35, 1))
        self.Light.node().setAttenuation(LVector3(.01, 0.01, 0.0))
        ...
        # self.trackNP.s
        # geometry
        loader.loadModel(globals.rel_path(None, path="/src/models/tracks/Road ML TT1SSNM.bam")).reparentTo(
            self.trackNP)
        ...
        directionalLightNP.setHpr(0, 20, -10)
